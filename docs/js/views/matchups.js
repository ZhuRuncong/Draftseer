...
export async function renderMatchups(root, params) {
  ...
  root.innerHTML = `
    <div class="toolbar">
      <label class="field">
        <span class="field-label">Ally role</span>
        <div class="field-row">
      ...
      <label class="field">
        <span class="field-label">Enemy role</span>
        <div class="field-row">
  ...
  function paint() {
    ...
    if (limit > 0) {
      // Restrict to top-N most-contested champions (pick rate + ban rate)
      // BEFORE the user's chosen sort, so we always look at the contested meta.
      const rowKeep = new Set(topByPopularity(allyRole,  rows, limit));
      const colKeep = new Set(topByPopularity(enemyRole, cols, limit));
